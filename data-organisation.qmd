...
The elements in organization are likely still of relevance, when working with code, but for the (re)usability it might make sense to include a reference on who to write good/clean code. 

We had a lecture on clean code at the 2024 Summer School: [https://osf.io/sbncz/files/akef3](https://osf.io/sbncz/files/akef3)
It mixes a little bit elements of documentation and organization. 
...
::: {.callout-important collapse="true" title="Solution: Example 5"}
In the deposit, there are two zip files, no folders allowed on Zenodo. However, within the zip files, there are folder structures:

- Ths is a code deposit and there are folders for the actual scripts concerning this research and separate folders for external tools required in the execution 
- the global README file is rather brief, but there are more in the subfolders explaining additional details. Still putting the dependencies into a "dependencies" folder could make this visible more intuitively. It is also not clear from the structure and README if the dependent software packages are fetched automatically or need to be manually added. 
:::
...
- The subfolders of `Data/Group` and `Data/Solo` have names like `01_09_2022__10_13_33`, which seem to refer to a date and maybe time of day.
...
- Length of the names: OK
- If folder name `01_09_2022__10_13_33` stands for timestamp 2022-09-01T10:13:33, then it could be renamed to `20220901T101333` or `2022-09-01_101333` to better follow general ISO standards.
:::

::: {.callout-important collapse="true" title="Solution: Example 5"}
- Yes, names are meaningful and intuitive.
- The subfolders are named after the dependencies, the main work folder is `_scripts` and files in there are called `runme_*`, one for each experiment.
- Subfolders do not really need an order, but the most important one `_scripts` is listed first, enforced by the underscore.
- No, spaces have been replaced by underscores and names use only ASCII characters.
- Length of the names: OK, although `_fish_ext_` in the runme names seems unnecessary in this context.
- Maybe shorten the runme names.
...
::: {.callout-important collapse="true" title="Solution: Example 5"}
- Results are strored as `.mat` files. A community standard. 
- Yes, suitable for long-term archiving.
- Fairly easy, readable with Python, Julia, Gnu Octave.
- Data files contain matrix data and `.mat` provides a naming scheme.
- `.mat` is optimized for performance although the actual performance may depend on the environment used. Still, the binary content already sets them well ahead of alternative ASCII formats in file size, reading time and accuracy of the stored data.
- The general format is easy to understand by humans.
- While originally being a proprietary format, readers (also Open Source) for all programming languages exist.
- Binary files like [HDF](https://de.wikipedia.org/wiki/Hierarchical_Data_Format) could be used, which could be equivalent to using the latest version of the `.mat` specification.
:::
...
::: {.callout-important collapse="true" title="Solution: Example 5"}
- All codes are ASCII files, data is binary.
- precision is that of the original data, since it is stored binary
- Special numbers could only appear following the IEE754 standard. (again due to binary storage)
- Time: stored as floating point numbers representing seconds (the standard in both Matlab and Python) 
- Tables: all standard in the used working environments.
- as everything is binary, the code is needed.
:::
